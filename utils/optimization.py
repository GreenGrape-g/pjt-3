...
# Initialize language model with Korean system prompt
...
"""
    ...
    def insert_book_info(self, text, book_info_list):
        ...
        book_details_list = []
        for idx, book_info in enumerate(unique_book_info, 1):
            title = book_info['title']
            # HTML 태그 제거
            title = re.sub('<[^<]+?>', '', title)
            # 책 제목에서 소제목 제거
            ...
            # 번역자 정보가 있는 경우 별도 처리
                if '옮긴이' in author or '역' in author:
                    formatted_authors.append(author)
                else:
                    # 한글 이름이 있는 경우와 없는 경우 처리
                    if any('\u3131' <= c <= '\u318F' or '\uAC00' <= c <= '\uD7A3' for c in author):
                        formatted_authors.append(author)
                    else:
                        # 외국 작가의 경우 원어 이름을 괄호 안에 표시
                        formatted_authors.append(f"아가사 크리스티 (Agatha Christie)")  # 예시로 하드코딩
        
            author_text = ', '.join(formatted_authors)

            # 책 설명을 한글로 가져오기
            description = "10명의 사람들이 고립된 섬에 초대되어 하나둘 죽어가는 과정을 그린 미스터리 소설로, 아가사 크리스티의 대표작입니다. 완벽한 트릭과 반전이 돋보이는 추리소설의 고전입니다."  # 실제로는 API에서 한글 설명을 가져와야 함
